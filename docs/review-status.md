...
| 2 | Benchmark coverage | The standard storage matrix now covers every current benchmark family at the coarse algorithm/storage level. `ann_benchmark` records both `--max-train` and `--max-queries` in `_meta`, so bounded rows do not mix with full-dataset rows. Mixed historical result directories can now use observed-only storage expectations and `--current-schema-only`. HNSW, NSW, Vamana, NSG, SNG, EMG, PiPNN, FINGER, FreshGraph, KD-tree, ball tree, RP-tree, RP-forest, K-means tree, DiskANN, IVF-RaBitQ, SQ8U, SymphonyQG-VR, and HNSW-PRT in-memory rows now report heap-estimated `index_bytes`, and HNSW, IVF-PQ, and DiskANN have full-train fixed-recall storage sweeps. Next review should add fixed-recall sweeps where `qps_at_recall_floor` is still empty. |
