...
- A 50K-vector classical sweep with broader tree settings now separates the
  ...
  recall at 310.8 QPS in memory and 316.7 QPS after snapshot load. Treat
  K-means tree as a controllable classical baseline; it now has a usable 95%+
  full-corpus row, but graph methods still lead.
