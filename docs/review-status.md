...
- Dataset-level benchmark summaries now have a first sampled profiler:
  ...
  Gini 0.925. A lighter pass now covers all ten locally converted standard
  datasets. GIST stood out with sampled LID p50 52.2 and hubness Gini 0.991;
  Deep Image showed 9.99M vectors, sampled LID p50 13.2, and hubness Gini 0.990.
  Both are warnings against treating the GloVe-25 curve as representative. Next
  profiler work is list/cluster imbalance and OOD split labels.
...
| 15 | Dataset difficulty metadata | First sampled profile script exists for VEC1/NBR1 datasets, and local profiles now cover SIFT, GloVe-25/50/100/200, Deep Image, NYTimes, Fashion-MNIST, MNIST, and GIST. Next review should add partition/list imbalance for IVF-style indexes and explicit OOD split labels where datasets provide them. |
