...
- Filtered-search selectivity sweeps now have a dedicated summarizer. Run
  ...
  RangeFiltered under-returns at narrow ranges.
...
| 9 | Filtered search | The selectivity JSONL summarizer exists and a 3K-vector, 200-query synthetic curve now exercises ACORN, FilteredGraph, RangeFiltered, and Curator with target counts, latency tails, returned counts, and ACORN two-hop counters. FilteredGraph, Curator, and RangeFiltered dense rows also emit in-memory heap `index_bytes`, but the dense rows are still unfiltered rows labeled `filter_mode=none`. Next review should tune ACORN and run fixed-recall/selectivity curves before making ACORN, FilteredGraph, RangeFiltered, or Curator QPS claims. |
