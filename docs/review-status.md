...
| Unsafe lint boundary | Passing | `unsafe_code = "warn"` is enabled at the crate level. Intentional unsafe is limited to the prefetch helper, architecture-specific PQ SIMD modules, and benchmark-only allocation counters; the aarch64 FastScan parity test calls the safe wrapper. |
