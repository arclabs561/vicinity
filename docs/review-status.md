...
| 11 | Sparse and late-interaction harnesses | SparseMIPS now canonicalizes unsorted sparse vectors by sorting dimensions and summing duplicate entries, so BM25/SPLADE-style emitters have a deterministic ingestion boundary. It still needs a SPLADE/BM25-style sparse dataset harness before QPS/recall rows matter. LEMUR needs training or reproducible model loading before storage or QPS rows matter. |
