...
| 4 | Dataset source pinning | GloVe-50 and MNIST now have direct SHA-256 pins. Add expected SHA-256 values for GloVe-200 and Deep Image after direct verification. Decide whether stable mirrors are needed beyond `ann-benchmarks.com`. |
...
| 8 | Classical methods | Corrupt-snapshot rejection now covers KD-tree, ball tree, RP-tree, RP-forest, and K-means tree, and docs no longer call KD/Ball exact. Next review should fill published benchmark rows for ball tree, RP-forest, K-means tree, and snapshot-loaded storage modes, then revisit dimensionality and metric gates. |
