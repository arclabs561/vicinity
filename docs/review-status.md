...
- Filtered-search selectivity sweeps now have a dedicated summarizer. Run
  ...
  reaches only 93.55% recall at 1% selectivity.
...
| 9 | Filtered search | The selectivity JSONL summarizer exists and a 3K-vector, 200-query synthetic curve now exercises ACORN, FilteredGraph, RangeFiltered, and Curator with target counts, latency tails, returned counts, and ACORN two-hop counters. FilteredGraph, Curator, and RangeFiltered dense rows also emit in-memory heap `index_bytes`, but the dense rows are still unfiltered rows labeled `filter_mode=none`. ACORN tuning now shows `ef_search=800`/128 two-hop clears 95% recall from 2% through 50% selectivity at about 3.8-4.0K QPS, but misses at 1%. Next review should implement or benchmark a selectivity-gated plan: tuned ACORN for moderate selectivity, pre-filtered exact/fallback candidates for very low selectivity, and no QPS claims without fixed-recall curves. |
