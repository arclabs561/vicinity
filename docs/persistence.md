...
| NSW / SNG / Vamana / NSG / FINGER / PiPNN | Yes, directory format | Yes | No | No | Build-once | Persists the built in-memory graph state and restores it directly. This is snapshot-memory persistence, not file-backed search. |
| EMG | No | Yes | No | No | Build-once | Add graph snapshot format after derived quantization state is audited. Reuse the shared graph snapshot helpers, not `segstore`. |
