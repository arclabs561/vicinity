...
Key insight: Queries should be perturbations of training vectors to ensure
meaningful nearest neighbor relationships exist.
...
def compute_difficulty_metrics(train: np.ndarray, test: np.ndarray) -> dict:
    ...
    hubness_skew = float(
        np.mean((hub_counts - hub_counts.mean()) ** 3) / (hub_counts.std() ** 3 + 1e-10)
    )
...
def save_labels(path: Path, labels: np.ndarray) -> None:
    ...
    atomic_write(
        path, b"LBL1" + struct.pack("<I", len(labels_u32)) + labels_u32.tobytes()
    )
...
def read_header(path: Path, magic: bytes, fmt: str) -> tuple[int, ...] | None:
    try:
        ...
        return None
    if (
        len(header) != len(magic) + struct.calcsize(fmt)
...
def output_manifest(scale_dir: Path, settings: dict) -> dict:
    ...
    for name, (magic, fmt, expected) in expected_headers(settings).items():
        ...
        if header != expected:
            raise ValueError(
                f"{name} header mismatch: expected {expected}, got {header}"
            )
        actual_bytes = path.stat().st_size
        if actual_bytes != expected_bytes[name]:
            raise ValueError(
                f"{name} size mismatch: expected {expected_bytes[name]}, "
                f"got {actual_bytes}"
            )
        outputs[name] = {"bytes": actual_bytes, "sha256": file_sha256(path)}
...
def matching_scale_outputs(scale_dir: Path, settings: dict) -> bool:
    ...
    for name, (magic, fmt, expected) in expected_headers(settings).items():
        ...
        if outputs.get(name, {}).get("bytes") != actual_bytes:
            return False
        if outputs.get(name, {}).get("sha256") != file_sha256(path):
            return False
    metrics_path = scale_dir / "metrics.json"
    return (
        metrics_path.exists()
        and outputs.get("metrics.json", {}).get("bytes") == metrics_path.stat().st_size
        and outputs.get("metrics.json", {}).get("sha256") == file_sha256(metrics_path)
    )
...
def generate_scale(scale: str, data_dir: Path, force: bool = False) -> dict:
    ...
    source_in_top10 = (
        np.sum([source_ids[i] in gt[i, :10] for i in range(n_test)]) / n_test
    )
    ...
    print(
        f"  Base: Cr={base_metrics['relative_contrast_mean']:.3f}, "
    ...
    test_drift = simulate_concept_drift(
        test, train_centroids, source_ids, train_topics, drift_strength=0.25, seed=200
    )
    ...
    print(
        f"  Drift: Cr={drift_metrics['relative_contrast_mean']:.3f}, "
    ...
    print(
        f"  Filter: {valid_filters}/{n_test} valid, "
        f"avg category size={avg_category_size:.0f}"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate multi-scale ANN benchmark data"
    )
    parser.add_argument(
        "--scale",
    ...
    parser.add_argument(
        "--force", action="store_true", help="Regenerate matching outputs"
    )
