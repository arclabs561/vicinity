...
//! per-segment HNSW searched in parallel and merged (the top-k merge is exact:
//! every global top-k vector ranks at or above its global rank within its own
//! segment). This is the deliberate alternative to a single evolving graph
//! (`fresh_graph` + FreshDiskANN consolidation): it trades single-graph
//! recall/latency at very large scale for native updatability and durability,
//! which fits a modest or frequently-churning corpus.
//!
...
/// vectors (a per-segment HNSW is built from the live ones per query).
...
impl UpdatableIndex {
    ...
    pub fn search(&self, query: &[f32], k: usize, ef: usize) -> Vec<(u32, f32)> {
        ...
        for seg in self.inner.segments() {
            cand.extend(self.search_batch(seg, &q, k, ef));
        }
        let buffered = self.inner.buffer().to_vec();
        cand.extend(self.search_batch(&buffered, &q, k, ef));
    ...
    fn search_batch(
        ...
        let mut any = false;
        for (id, v) in batch {
            // Vectors are stored already-normalized.
        ...
        idx.search(query, k, ef).unwrap_or_default()
    }
}
